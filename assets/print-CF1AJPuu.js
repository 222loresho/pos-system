const u="POS-80C";function x(){return new Promise((t,a)=>{if(window.qz&&window.qz.websocket.isActive()){t();return}window.qz.websocket.connect().then(t).catch(a)})}async function m(t){try{await x();const a=window.qz.configs.create(u),e=t.map(o=>({type:"raw",format:"plain",data:o+`
`}));await window.qz.print(a,e)}catch(a){return console.error("QZ print failed, falling back to browser print",a),!1}return!0}function r(t){return"\x1B"+t}const c=r("@"),d=r("a"),h=r("a\0"),p=r("E"),l=r("E\0"),w="VA",s="-".repeat(42),b=`


`;function n(t,a,e=42){const o=e-t.length-a.length;return t+" ".repeat(Math.max(1,o))+a}const v=async t=>{const a=new Date(t.created_at),e=[c,d+p+"Triple Two Loresho"+l,d+"Loresho, Nairobi",h+s,n("Order: "+t.order_number,"Table: "+t.table_name),n("Waiter: "+t.waiter_name,a.toLocaleDateString()),n("",a.toLocaleTimeString()),s,n("Item","Amount"),s,...t.items.map(i=>n(i.product_name+" x"+i.quantity,"KSh "+i.subtotal)),s,p+n("TOTAL","KSh "+t.total)+l,s,d+"Please present this bill to the cashier",b,w];await m(e.join(`
`))||g(t)},f=async t=>{const a=t.paymentMethod==="mpesa"?"Mpesa":t.paymentMethod==="card"?"Card":"Cash",e=[c,d+p+"Triple Two Loresho"+l,d+"Loresho, Nairobi",h+s,t.orderNumber?n("Order: "+t.orderNumber,"Table: "+(t.tableName||"-")):"",n("Cashier: "+t.cashier,t.date.split(",")[0]),n("Waiter: "+t.waiterName,t.date.split(",")[1]?.trim()||""),s,n("Item","Amount"),s,...t.items.map(i=>n(i.product_name+" x"+i.quantity,"KSh "+i.subtotal)),s,p+n("TOTAL","KSh "+t.total)+l,s,n("Payment:",a),...t.paymentMethod==="cash"?[n("Amount Paid:","KSh "+t.amountPaid),n("Change:","KSh "+t.change)]:[],...t.paymentMethod==="mpesa"?[n("Mpesa Code:",t.mpesaCode)]:[],...t.paymentMethod==="card"?[n("Auth No:",t.cardAuth)]:[],s,d+"Thank you for your visit!",d+"Please come again",s,d+"Note to Waiter: Submit this receipt",d+"attached with the bill to the cashier.",b,w];await m(e.join(`
`))||y(t)};function g(t){const a=window.open("","_blank","width=400,height=600"),e=new Date(t.created_at),o=t.items.map(i=>`<tr><td>${i.product_name}</td><td>x${i.quantity}</td><td class="right">KSh ${i.subtotal}</td></tr>`).join("");a.document.write(`<html><head><title>Bill</title>
  ...
  </body></html>`),a.document.close(),setTimeout(()=>{a.focus(),a.print()},500)}function y(t){const a=window.open("","_blank","width=400,height=600"),e=t.items.map(o=>`<tr><td>${o.product_name}</td><td>x${o.quantity}</td><td class="right">KSh ${o.subtotal}</td></tr>`).join("");a.document.write(`<html><head><title>Receipt</title>
  ...
  </body></html>`),a.document.close(),setTimeout(()=>{a.focus(),a.print()},500)}export{v as printBill,f as printReceipt};
