...
export default function CashierBills({ user, onLogout, onSwitchToPOS }) {
  ...
  return (
    <div className="page">
      ...
      {loading && <p className="text-muted" style={{padding:'14px'}}>Loading...</p>}

      {tab === 'submitted' && (
        <div style={{padding:'14px'}}>
          {submitted.length === 0 && <div className="card" style={{textAlign:'center',padding:'32px'}}><div style={{fontSize:'40px'}}>📭</div><div className="text-muted">No submitted bills yet</div></div>}
          {submitted.map(o => (
            <div key={o.id} className="cleared-card" style={{border:'1px solid var(--accent)'}}>
              ...
              <div className="text-sm text-muted mb-8">🕐 {fmtTime(o.submitted_at)}</div>
              ...
              <div style={{background:'var(--card)',borderRadius:'8px',padding:'8px',marginBottom:'10px'}}>
                <div className="text-sm text-bold mb-8">💳 Payment</div>
                {o.payment_details && o.payment_details.length > 0 ? (
                  o.payment_details.map((s, i) => (
                    <div key={i} className="flex-between text-sm mb-8">
                      <span>{methodIcon(s.method)} {s.method} {s.ref ? `(${s.ref})` : ''}</span>
              ...
              <div className="flex gap-8">
                ...
                <button className="btn" style={{flex:1,background:'var(--red)',color:'white'}} onClick={() => { setRejectModal(o); setRejectNote(''); }}>❌ Reject</button>
              </div>
            </div>
          ))}
        </div>
      ...
      {tab === 'confirmed' && (
        <div style={{padding:'14px'}}>
          {confirmed.length === 0 && <div className="card" style={{textAlign:'center',padding:'32px'}}><div style={{fontSize:'40px'}}>📭</div><div className="text-muted">No confirmed bills yet</div></div>}
          {confirmed.map(o => (
        ...
        <div className="modal-overlay">
          <div className="modal">
            ...
            <div className="modal-actions">
              <button className="btn" style={{background:'var(--red)',color:'white'}} onClick={reject}>❌ Reject</button>
