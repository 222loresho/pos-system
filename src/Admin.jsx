...
export default function Admin({ user, onLogout, onSwitchToPOS }) {
  ...
  const tabs = [
    ...
    { id: 'lowstock', label: lowStockCount > 0 ? `⚠️ Stock (${lowStockCount})` : '⚠️ Stock' },
  ];
      ...
      {tab === 'products' && (
        <div style={{padding:'14px'}}>
          <div className="card">
            ...
            <button className="btn btn-primary" style={{width:'100%'}} onClick={addProduct}>Add Product</button>
          </div>
      ...
      {tab === 'categories' && (
        <div style={{padding:'14px'}}>
          <div className="card">
            ...
            <button className="btn btn-primary" style={{width:'100%'}} onClick={addCategory}>Add Category</button>
          </div>
          {categories.map(c => <div key={c.id} className="card">{c.name}</div>)}
        </div>
      )}
