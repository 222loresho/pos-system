        ...
        <svg width={chartW + 40} height={chartH + 60} style={{ display: 'block' }}>
          {[0, 0.25, 0.5, 0.75, 1].map((pct, i) => (
            <g key={i}>
              ...
              <text x={0} y={chartH - pct * chartH + 14} fill="#888" fontSize="9" textAnchor="start">{Math.round(maxVal * pct / 1000)}k</text>
            </g>
          ))}
          {data.data.map((d, i) => {
            ...
            return (
              <g key={i}>
                <rect x={x} y={y} width={barW} height={barH} rx="3"
                  fill={isPeak ? '#e94560' : d.revenue > 0 ? '#4caf50' : 'rgba(255,255,255,0.1)'} />
                {d.revenue > 0 && (
                  <text x={x + barW / 2} y={y - 4} fill="#ccc" fontSize="8" textAnchor="middle">
                    {d.revenue >= 1000 ? `${(d.revenue/1000).toFixed(1)}k` : d.revenue}
                  </text>
                )}
                <text x={x + barW / 2} y={chartH + 24} fill="#888" fontSize="9" textAnchor="middle"
                  transform={period === 'monthly' ? `rotate(-45, ${x + barW/2}, ${chartH + 24})` : ''}>
                  {fmtLabel(d.label)}
                </text>
              </g>
            );
  ...
  return (
    <div>
      <div className="flex-between mb-8">
        <div className="section-title" style={{ margin: 0 }}>📈 Revenue Chart</div>
        ...
        <>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(130px, 1fr))', gap: '10px', marginBottom: '16px' }}>
            <div className="card card-dark" style={{ textAlign: 'center' }}>
              ...
              <div className="text-accent text-bold" style={{ fontSize: '18px', margin: '4px 0' }}>KSh {data.total.toLocaleString()}</div>
            </div>
            {data.peak && data.peak.revenue > 0 && (
              <div className="card card-dark" style={{ textAlign: 'center' }}>
                <div className="text-muted text-sm">Peak {period === 'daily' ? 'Hour' : 'Day'}</div>
                <div className="text-bold" style={{ fontSize: '14px', margin: '4px 0' }}>{fmtLabel(data.peak.label)}</div>
          ...
          <div className="card card-dark">
            {data.data.every(d => d.revenue === 0)
              ? <p className="text-muted" style={{ textAlign: 'center', padding: '32px 0' }}>No sales data for this period</p>
              : renderChart()}
          </div>
        </>
      )}
