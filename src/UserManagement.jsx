...
export default function UserManagement() {
  ...
  const saveUser = async () => {
    ...
    if (!editUser && !form.pin) return setMessage('❌ PIN required!');
  ...
  const deleteUser = async (user) => {
    ...
    try {
      ...
      setMessage('✅ User deleted!'); fetchUsers();
    } catch (e) { setMessage('❌ ' + (e.response?.data?.error || 'Failed')); }
  };
  ...
  const roleColor = (role) => role === 'admin' ? 'var(--red)' : role === 'waiter' ? '#f0a500' : 'var(--accent)';
      ...
      {users.map(u => (
        <div key={u.id} className="admin-item" style={{opacity: u.active ? 1 : 0.5}}>
          ...
          <div className="admin-item-actions">
            ...
            <button className="btn btn-sm" style={{background:u.active?'#555':'var(--accent)',color:u.active?'white':'#0a0a0f'}} onClick={() => toggleActive(u)}>{u.active ? '🔒' : '✅'}</button>
